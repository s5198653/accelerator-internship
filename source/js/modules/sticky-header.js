export class StickyHeader {
  constructor() {
    ...
    this._headerHeight = this._stickyHeader
      ? this._stickyHeader.offsetHeight
      : '';
    this._hidePoint = this._stickyHeader
      ? +this._stickyHeader.dataset.hidePoint
      : 0;
  ...
  _onWindowScroll() {
    ...
    if (
      this._checkScrollDirection() === 'down' &&
      this._scrollY > this._hidePoint
    ) {
      this._hideHeader();
    }

    if (
      this._checkScrollDirection() === 'up' ||
      this._scrollY <= this._hidePoint
    ) {
      this._showHeader();
    }
  ...
  _checkTheme() {
    this._themeItems.forEach((item) => {
      if (
        ...
        item.getBoundingClientRect().height + item.getBoundingClientRect().top >
          0
      ) {
